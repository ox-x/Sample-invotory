...
public class WirelessLogServer {
    ...
    private String buildWarehousePage() {
        ...
        String page = "<!DOCTYPE html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
                ...
                + "function filterItems(){var q=(document.getElementById('searchInput').value||'').toLowerCase();document.querySelectorAll('.item-group').forEach(function(g){var ms=q===''||(g.dataset.name||'').toLowerCase().includes(q)||(g.dataset.epc||'').toLowerCase().includes(q)||(g.dataset.desc||'').toLowerCase().includes(q);g.querySelectorAll('.child-row').forEach(function(c){var cm=q===''||(c.dataset.name||'').toLowerCase().includes(q)||(c.dataset.epc||'').toLowerCase().includes(q)||(c.dataset.desc||'').toLowerCase().includes(q);var cf=currentFilter==='all'||c.dataset.status===currentFilter;c.style.display=cm&&cf?'':'none';if(cm)ms=true;});var fm=currentFilter==='all'||g.dataset.status===currentFilter;g.style.display=ms&&fm?'':'none';});}"
    ...
    private String buildLogsPage() {
        ...
        return "<!DOCTYPE html><html><head>"
                ...
                + ".type-borrow,.type-\\u501F\\u51FA{background:#43A047}"
                + ".type-return,.type-\\u5F52\\u8FD8{background:#FB8C00}"
